...
public class EmployeeSignUpServiceImpl implements EmployeeSignUpSerivice {
    ...
    public void addEmployeeSignUp(Employee employee) {
        ...
        try {
            String hashPassword = BCrypt.hashpw(employee.getEmployeePassword(),BCrypt.gensalt());
            employee.setEmployeePassword(hashPassword);

            employeeSignUpRepository.employeeSignUp(employee);
            userLoginService.employeeLogin(employee);
        } catch (Exception e) {
